...
const SortableTable: React.FC = () => {
  ...
  const [sortKey, setSortKey] = useState<keyof IDevice | null>(null);
  ...
  const updateTableHeight = () => {
    ...
    const titleHeight = document.querySelector('nav')?.offsetHeight || 0;
    const offset = 140; // Margin or padding offsets
          ...
          >
              {paginatedDevices.map((device) => (
                <TableRow key={device.id}>
                  {Object.values(device).map((value: any, index) => (
